'''
Tenable API Script to list all Target Groups and Tags
...
def main():
  ...
  strOutFile = strOutPath + "TagValues.csv"
  objFileOut = open(strOutFile,"w")
  objFileOut.write("Category,Value,Type,ID\n")
  ...
  if "values" in APIResponse:
    if isinstance(APIResponse["values"],list):
        # iTagSize = len(APIResponse["values"])
        for dictValue in APIResponse["values"]:
          strValueID = dictValue["uuid"]
          strValue = dictValue["value"]
          strCategory = dictValue["category_name"]
          strType = dictValue["type"]
          objFileOut.write("{},{},{},{}\n".format(strCategory,strValue,strType,strValueID))
          # LogEntry ("Processing tag value {} with ID {}. Tag {} out of {}".format(strValue,strValueID,iRowCount,iTagSize))
          iRowCount += 1
    else:
  ...
  strURL = strBaseURL + strAPIFunction
  LogEntry("Now Pulling all Target Groups")
  APIResponse = MakeAPICall(strURL,strHeader,strMethod, dictPayload)
